...
func CreateNamespaceIfNotExist(client clientset.Interface, ns *corev1.Namespace) error {
	if _, err := client.CoreV1().Namespaces().Create(context.TODO(), ns, metav1.CreateOptions{}); err != nil {
		if !apierrors.IsAlreadyExists(err) {
			return errors.Wrap(err, "unable to create namespace")
		}
...
func CreateOrUpdateIngress(client clientset.Interface, ing *netv1.Ingress) error {
	if _, err := client.NetworkingV1().Ingresses(ing.ObjectMeta.Namespace).Create(context.TODO(), ing, metav1.CreateOptions{}); err != nil {
		if !apierrors.IsAlreadyExists(err) {
			return errors.Wrap(err, "unable to create ingress")
		}

		if _, err := client.NetworkingV1().Ingresses(ing.ObjectMeta.Namespace).Update(context.TODO(), ing, metav1.UpdateOptions{}); err != nil {
...
func CreateIngressIfNotExist(client clientset.Interface, ing *netv1.Ingress) error {
	if _, err := client.NetworkingV1().Ingresses(ing.ObjectMeta.Namespace).Create(context.TODO(), ing, metav1.CreateOptions{}); err != nil {
		if !apierrors.IsAlreadyExists(err) {
			return errors.Wrap(err, "unable to create ingress")
		}
...
func AppendPathToIngress(client clientset.Interface, namespace, name string, path netv1.HTTPIngressPath) error {
